...
def calcular_salud_financiera(morosidad, liquidez, roa, roe, cobertura):
    ...
    Retorna: puntaje_total, categoria, clase_css, desglose_puntajes
    """
    ...
    return puntaje_total, categoria, clase_css, desglose
        ...
        with st.spinner("🔮 Realizando predicción y análisis..."):
            ...
            if len(df_ruc) < 12:
                ...
                for _, row in df_future.iterrows():
                    puntaje, categoria, clase, desglose = calcular_salud_financiera(
                ...
                # Áreas de categorías
                ax_salud.axhspan(80, 100, alpha=0.15, color='green', label='Excelente')
                ax_salud.axhspan(60, 80, alpha=0.15, color='blue', label='Buena')
                ax_salud.axhspan(40, 60, alpha=0.15, color='orange', label='Regular')
                ax_salud.axhspan(20, 40, alpha=0.15, color='red', label='Deficiente')
                ax_salud.axhspan(0, 20, alpha=0.15, color='darkred', label='Crítica')
                        ...
                        with col_health:
                            ...
                            st.markdown(
                                ...
                                <div class="metric-card {salud_mes['Clase']}" style="margin-top: 0;">
                                    <h2 style="margin: 0; font-size: 2.5rem;">{salud_mes['Puntaje']:.1f}</h2>
                                    <p style="margin: 0.5rem 0 0 0; font-size: 1.2rem;"><strong>{salud_mes['Categoría']}</strong></p>
                                    <p style="margin: 0.5rem 0 0 0; font-size: 0.9rem;">puntos</p>
                                </div>
                                """,
                                unsafe_allow_html=True
                            )
                            ...
                            recomendaciones = {
                                ...
                                "Buena": ("📈", "Mantener estrategias, buscar mejoras"),
                                "Regular": ("⚠️", "Revisar procesos, mejorar áreas débiles"),
                                "Deficiente": ("🔴", "Plan correctivo necesario"),
                                "Crítica": ("🚨", "Intervención urgente requerida")
                            }

                            icono, texto = recomendaciones[salud_mes['Categoría']]
...
with col_info1:
    with st.expander("ℹ️ Metodología del Índice de Salud Financiera"):
        ...
        **Fórmula:**
        ```
        Salud = (Morosidad × 30%) + (Liquidez)
        ```
        Este es un ejemplo de cómo se combinan los indicadores para obtener el puntaje final.
